 ...
 * Reader 1: pins 2,3
 * Reader 1 aux pins
   Red/green/buzzer: 4,5,6
 * Reader 2: pins 19,18
 * Reader 2 aux pins:
 * Red/green/buzzer: 17,7,8
...
// #define super_user1    0x1234567
// #define super_user2   0x17654321                  // Name and badge number in HEX. We are not using checksums or site ID, just the whole
// const long  superUserList[] = { super_user1, super_user2 };  // Super user table (cannot be changed by software)
const long  superUserList[] = { };  // Super user table (cannot be changed by software)

#define PRIVPASSWORD 0x123456             // Console "priveleged mode" password

#define DOORDELAY 5000                  // How long to open door lock once access is granted. (2500 = 2.5s)
#define SENSORTHRESHOLD 15             // Analog sensor change that will trigger an alarm (0..255)
...
#define REDLED1 4                       // Define the pin for green LED on reader 1
#define REDLED2 6                       // Define the pin for green LED on reader 2
...
const prog_uchar doorChimeMessage[]       PROGMEM  = {"Door opened."};
...
const prog_uchar statusMessage3[]         PROGMEM  = {"Battery Voltage:"}; 
const prog_uchar statusMessage4[]         PROGMEM  = {"Front door open state (0=closed):"};
const prog_uchar statusMessage5[]         PROGMEM  = {"Rear door open state (0=closed):"};
const prog_uchar statusMessage6[]         PROGMEM  = {"Front 2nd door open state (0=closed):"};
const prog_uchar statusMessage7[]         PROGMEM  = {"Common area door open state (0=closed):"};
const prog_uchar statusMessage8[]         PROGMEM  = {"Front door unlocked state(1=locked):"};
const prog_uchar statusMessage9[]         PROGMEM  = {"Rear door unlocked state(1=locked):"};
const prog_uchar statusMessage10[]        PROGMEM  = {"Input Status:"};

const prog_uchar motionSensor0[]	  PROGMEM  = {"Motion Sensor 0 (front)"};
const prog_uchar motionSensor1[]	  PROGMEM  = {"Motion Sensor 1 (danger room)"};
const prog_uchar motionSensor2[]	  PROGMEM  = {"Motion Sensor 2 (prototyping)"};
const prog_uchar motionSensor3[]	  PROGMEM  = {"Motion Sensor 3 (classroom)"};

const prog_uchar doorSensor4[]		  PROGMEM  = {"Sensor 4 (front door)"};
const prog_uchar doorSensor5[]		  PROGMEM  = {"Sensor 5 (rear door)"};
const prog_uchar doorSensor6[]		  PROGMEM  = {"Sensor 6 (2nd front door)"};
const prog_uchar doorSensor7[]		  PROGMEM  = {"Sensor 7 (common area door)"};

const prog_uchar glassSensor8[]		  PROGMEM  = {"Glass Break Sensor 8 (break)"};
const prog_uchar glassSensor9[]		  PROGMEM  = {"Glass Break Sensor 9 (tamper)"};

const prog_uchar powerSensor12[]	  PROGMEM  = {"Power Supply Sensor 12 (batt ok)"};
const prog_uchar powerSensor13[]	  PROGMEM  = {"Power Supply Sensor 13 (ac ok)"};
...
void logalarmSensor(uint8_t zone)
{
	...
	switch(zone)
        ...
        case 0:
          PROGMEMprint(motionSensor0);
	  break;
        case 1:
          PROGMEMprint(motionSensor1);
	  break;
        case 2:
          PROGMEMprint(motionSensor2);
	  break;
        case 3:
          PROGMEMprint(motionSensor3);
	  break;
        case 4:
          PROGMEMprint(doorSensor4);
	  break;
        case 5:
          PROGMEMprint(doorSensor5);
	  break;
        case 6:
          PROGMEMprint(doorSensor6);
	  break;
        case 7:
          PROGMEMprint(doorSensor7);
	  break;
        case 8:
          PROGMEMprint(glassSensor8);
	  break;
        case 9:
          PROGMEMprint(glassSensor9);
	  break;
        case 10:
	  ...
	  break;
        case 12:
          PROGMEMprint(powerSensor12);
	  break;
        case 13:
          PROGMEMprint(powerSensor13);
	  break;
        case 14:
                 ...
                 if(privmodeEnabled==true) {
                  ...
                  if(privmodeEnabled==true) {
                   ...
                   case '9': {                                            // Show site status
					...
					Serial.println(pollAlarm(4),DEC);
					PROGMEMprint(statusMessage5);
					Serial.println(pollAlarm(5),DEC);                  
					PROGMEMprint(statusMessage6);
					Serial.println(pollAlarm(6),DEC);
					PROGMEMprint(statusMessage7);
					Serial.println(pollAlarm(7),DEC);                  

					PROGMEMprint(statusMessage8); 
					Serial.println(door1Locked);                    
					PROGMEMprint(statusMessage9); 
					...
					// Serial.println('V');
					Serial.print((0.052*analogRead(15)),2);          // Pin 15 has a 4:1 voltage divider. Reads 0..1024 for 0..20V input at terminals.
					Serial.println('V');

					Serial.println(' ');
					PROGMEMprint(statusMessage10);
					Serial.println(' ');
					trainAlarm(1);
					Serial.println(' ');
